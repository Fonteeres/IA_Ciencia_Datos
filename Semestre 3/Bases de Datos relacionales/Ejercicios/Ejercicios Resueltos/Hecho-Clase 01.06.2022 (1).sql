...
SELECT * FROM marcas where nombre like '%B52%'
--EJERCICIO HECHO POR GINO SIBELLO
SELECT
	DISTINCT
	v.nombre,
	v.ingreso
FROM
	...
	inner join vendet vt 
	on (vt.letra = vc.letra and vt.factura = vc.factura)
	inner join articulos art 
	on (art.articulo = vt.articulo)
	inner join sucursales sc 
	on (vc.sucursal = sc.sucursal)
	inner join marcas mc 
	on (art.marca = mc.marca)
WHERE
	sc.denominacion LIKE '%PATIO OLMOS%'and
	mc.nombre like '%B52%' and
	vc.anulada = 0
order by
	v.nombre asc
...
SELECT * FROM articulos a inner join marcas m on (a.marca = m.marca) where m.marca = 'A'
...
SELECT 'Valor: ' + STR(5) AS "Resultado"
...
--EJERCICIO HECHO POR GINO SIBELLO
SELECT * FROM vencab
SELECT * FROM vendet where precio < precioventa - (precioventa * 10 / 100)
SELECT * FROM vendedores
SELECT * FROM articulos
SELECT * FROM sucursales
SELECT
...
FROM
	vendedores v
	inner join vencab vc on (v.vendedor = vc.vendedor)
	inner join vendet vd on (vc.factura = vd.factura and vc.letra = vc.letra)
	inner join articulos a on (a.articulo = vd.articulo)
	inner join sucursales s on (s.sucursal = vc.sucursal)
where
	YEAR(vc.fecha) = 2005
	and MONTH(vc.fecha) = 9
	and vd.precio < vd.precioventa * 0.9
	and vc.anulada = 0
